...
class FileService:
    def __init__(self, db: AsyncSession):
        ...
        # Define specific directories as Path objects for easier manipulation
        self.avatars_dir = self.upload_dir / "avatars"
        self.group_avatars_dir = self.upload_dir / "avatars" / "groups" # <--- KOREKSI: Definisi path yang benar
        self.attachments_images_dir = self.upload_dir / "attachments" / "images"
        self.attachments_videos_dir = self.upload_dir / "attachments" / "videos"
        self.attachments_audio_dir = self.upload_dir / "attachments" / "audio"
        self.attachments_documents_dir = self.upload_dir / "attachments" / "documents"
    ...
    def _create_directories(self):
        ...
        directories = [
            self.upload_dir,
            self.avatars_dir,
            self.group_avatars_dir, # Ensure this is created
            self.attachments_images_dir,
            self.attachments_videos_dir,
            self.attachments_audio_dir,
            self.attachments_documents_dir,
            self.thumbnails_dir
        ]
    ...
    async def create_thumbnail(self, file_path_relative: str, file_type: FileType, file_content: bytes) -> Optional[str]:
        ...
        if file_type == FileType.IMAGE:
            try:
                ...
                thumbnail_full_path = self.thumbnails_dir / thumbnail_filename # Use self.thumbnails_dir
    ...
    async def _create_video_thumbnail(self, file_path_relative: str) -> Optional[str]:
        ...
        thumbnail_filename = f"thumb_{uuid.uuid4()}.jpg"
        thumbnail_path = self.thumbnails_dir / thumbnail_filename # Use self.thumbnails_dir

        # Ensure thumbnails directory exists
        self.thumbnails_dir.mkdir(parents=True, exist_ok=True) # Use self.thumbnails_dir.mkdir
    ...
    async def upload_avatar(self, file: UploadFile, user_id: str) -> FileUploadResponse:
        ...
        file_path_relative = await self.save_file(
            ...
            file.filename,
            "avatars" # This will save to self.upload_dir / "avatars"
        )
        ...
        if user.profile_picture:
            try:
                # Extract relative path from existing URL
                old_relative_path_str = Path(user.profile_picture).relative_to(f"{settings.BASE_URL}/uploads/")
                # Ensure it's in the 'avatars' directory to prevent arbitrary file deletion
                if old_relative_path_str.parts[0] == "avatars":
                    await self._delete_file_if_exists(str(old_relative_path_str))
                    # Attempt to delete old thumbnail if it followed the pattern
                    if len(old_relative_path_str.parts) > 1 and old_relative_path_str.parts[1].startswith("user_avatar_"):
                        # Assuming thumbnail name structure like "thumb_user_avatar_UUID.jpg"
                        old_thumb_name = "thumb_" + old_relative_path_str.name.replace(old_relative_path_str.suffix, ".jpg")
                        await self._delete_file_if_exists(str(Path("thumbnails") / old_thumb_name))
            except ValueError:
                # old_relative_path is not relative to expected BASE_URL/uploads/, ignore deletion
                print(f"Warning: Old profile picture URL '{user.profile_picture}' is not a local upload, skipping deletion.")
            except Exception as e:
                print(f"Error trying to delete old avatar or thumbnail: {e}")
    ...
    async def upload_group_avatar(self, file: UploadFile) -> FileUploadResponse: 
        """Uploads a group avatar file and returns FileUploadResponse."""
        validation_result = await self.validate_file(file)

        if validation_result["file_type"] != FileType.IMAGE:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid file type. Only images are allowed for group avatars."
            )

        file_extension = Path(file.filename).suffix.lower()
        unique_filename = f"group_avatar_{uuid.uuid4()}{file_extension}"
        file_path = self.group_avatars_dir / unique_filename

        try:
            async with aiofiles.open(file_path, 'wb') as f:
                await f.write(validation_result["content"])

            relative_path_from_uploads = Path("avatars") / "groups" / unique_filename
            file_url = self.get_file_url(str(relative_path_from_uploads))

            # Kembalikan FileUploadResponse langsung
            return FileUploadResponse(
                id=str(uuid.uuid4()),
                file_url=file_url,
                file_type=validation_result["file_type"], 
            ...
            raise HTTPException(
                ...
                detail=f"Failed to upload group avatar: {e}"
            )
